    ...
    for (size_t idx = start; idx + 0 < lines.size(); idx += 3) {
        ...
        if (mooreCoincideEnTexto(userLine, patron)) {
            ...
            if (!bankLine.empty()) {
                ...
                if (!comp.empty()) {
                    string dec = LZW_decompress(comp);
                    if (dec != "(error de LZW)") bank = descifrarVigenere(dec, claveVigenere);
                    else bank = "(error de LZW)";
                }
                else {
...
int main() {
    ...
    if (contrasena.size() != 4) {
        cout << "La contrasena no coincide con el formato requerido (4 caracteres)." << endl;
        return -1;
    }
    else {
